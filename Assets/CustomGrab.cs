...
public class CustomGrab : MonoBehaviour
{
    ...
    void Update()
    {
        if (doubleRotationAction.action.IsPressed()) {
        ...
        if (grabbing)
        {
            ...
            if (grabbedObject)
            {
                ...
                Vector3 jepPosition = grabbedObject.position - transform.position;
                Quaternion deltaRotation = transform.rotation * Quaternion.Inverse(previousRotation);
                jepPosition = deltaRotation * jepPosition - jepPosition;
                Vector3 deltaPosition = transform.position - previousPosition + jepPosition;
